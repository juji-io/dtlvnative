...
powershell -Command "(Get-Content -Raw 'src\usearch\c\usearch.h') -replace '(?i)(USEARCH_EXPORT\s+typedef\s+struct\s+usearch_init_options_t\s+\{)', '#pragma pack(push,4)' + [Environment]::NewLine + '$1' | Set-Content 'src\usearch\c\usearch.h'"
